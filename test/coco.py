...
def apply_detection(image, nnet, scales, decode_func,
                    top_k, avg, std, categories, merge_bbox,
                    max_per_image=100,
                    ae_threshold=0.5,
                    nms_kernel=3, nms_algorithm=2, nms_threshold=0.45,
                    weight_exp=1):
    ...
    for scale in scales:
        ...
        # do detection
        dets, center = decode_func(nnet, images, top_k,
    ...
    for j in range(categories):
        ...
        if merge_bbox:
            soft_nms_merge(final_bboxes[j + 1], Nt=nms_threshold,
                           method=nms_algorithm, weight_exp=weight_exp)
        else:
            soft_nms(final_bboxes[j + 1], Nt=nms_threshold,
                     method=nms_algorithm)
        final_bboxes[j + 1] = final_bboxes[j + 1][:, 0:5]

    scores = np.hstack([final_bboxes[j][:, -1] 
...
def kp_detection(db, nnet, result_dir, debug=False, decode_func=kp_decode):
    ...
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        ...
        detections = apply_detection(image, nnet, scales, decode_func,
                                     top_k=db.configs['top_k'],
                                     categories=db.configs['categories'],
                                     merge_bbox=db.configs['merge_bbox'],
                                     max_per_image=db.configs["max_per_image"],
                                     ...
                                     weight_exp=db.configs['weight_exp'],
                                     nms_threshold=db.configs['nms_threshold'],
                                     avg=db.mean, std=db.std,
                                     ae_threshold=db.configs["ae_threshold"],
                                     nms_kernel=db.configs["nms_kernel"])

        top_bboxes[image_id] = detections
        if debug:
            image_file = db.image_file(db_ind)
            image      = cv2.imread(image_file)
            im         = image[:, :, (2, 1, 0)]
